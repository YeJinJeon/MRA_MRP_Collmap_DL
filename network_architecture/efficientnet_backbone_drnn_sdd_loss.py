...
class EfficientnetBackboneDrnnSddLoss(nn.Module):
    ...
    def __init__(self, n_channels, growth_rate, reduction, k_ordinal_class, dev_0, dev_1, dev_2, dev_3, model_name, scale_factor=(1, 14, 14)):
        ...
        self.dev_3 = dev_3
        # dense 1
        self.efficient_net = EfficientNet.from_name(model_name, dev_0, dev_2, out_classes=1, in_channels=n_channels)
        self.scene_understanding_module = SceneUnderstandingModule(1280, scale_factor, device=dev_2).to(dev_2)
        # # resblock
        n_channels = 1280
        self.transpose_conv1 = nn.ConvTranspose3d(n_channels, int(n_channels * reduction), kernel_size=(3, 2, 2), stride=(1, 2, 2), padding=(1, 0, 0)).to(dev_2)
        self.resblock1 = ResidualBlock(752).to(dev_2)
        n_channels = 1504
        # n_channels = int(n_channels * 6) # n_channels = 512
        self.transpose_conv2 = nn.ConvTranspose3d(n_channels, 120, kernel_size=(3, 2, 2), stride=(1, 2, 2), padding=(1, 0, 0)).to(dev_2)
        self.resblock2 = ResidualBlock(160).to(dev_2)
        n_channels = 320
        self.transpose_conv3 = nn.ConvTranspose3d(n_channels, 40, kernel_size=(3, 2, 2), stride=(1, 2, 2), padding=(1, 0, 0)).to(dev_2)
        # n_channels = int(n_channels // 12)  # n_channels = 32
        self.resblock3 = ResidualBlock(64).to(dev_2)
        self.transpose_conv4 = nn.ConvTranspose3d(128, 8, kernel_size=(3, 2, 2), stride=(1, 2, 2), padding=(1, 0, 0)).to(dev_0)
        # n_channels = int(n_channels // 12) # n_channels = 8
        self.resblock4 = ResidualBlock(24).to(dev_0)
        ...
        self.orl1 = OrdinalRegressionLayer().to(dev_0)
        self.last_conv = nn.Conv3d(48, 5, kernel_size=(1, 1, 1)).to(dev_0)

    def forward(self, x):
        # encoder
        x = x.to(self.dev_2, dtype=torch.float)
        ...
        reduction_1, reduction_2, reduction_3, reduction_4, out = reduction_1.to(self.dev_0), reduction_2.to(self.dev_2), \
                                                                  reduction_3.to(self.dev_2), reduction_4.to(self.dev_2), \
                                                                  out.to(self.dev_2)
        ...
        reduction_1 = reduction_1.to(self.dev_0)
        ...
        #
        # out = self.ordinal_regression_conv1(out)
        # ord_reg_output = self.orl1(out)
        #
        return (reg_output.to(self.dev_0), out)
