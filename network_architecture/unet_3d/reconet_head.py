...
class TGMandTRM(nn.Module):
    ...
    def ConvGeneration(self, rank, h, s):
        ...
        for _ in range(0, rank):
                ...
                nn.Conv3d(256, 256 // n, kernel_size=1, bias=False),
                nn.Sigmoid(),
            ))
        ...
        if s != 0:
            ...
            for _ in range(0, rank):
                    ...
                    nn.Conv3d(s, s // n, kernel_size=1, bias=False),
                    nn.Sigmoid(),
                ))
        ...
        for _ in range(0, rank):
                ...
                nn.Conv3d(h, h // n, kernel_size=1, bias=False),
                nn.Sigmoid(),
            ))
    ...
    def TukerReconstruction(self, batch_size, s, h, ps, feat, feat1, feat2, feat3):
        ...
        if s != 0 and feat1 != None:
            ...
            recon_C = feat.view(b, -1, ps)
            recon_H = feat2.view(b, -1, ps)
            recon_W = feat3.view(b, ps, -1)
            CHW = torch.bmm(recon_C, torch.bmm(recon_H, recon_W).view(b, ps * ps, -1)).view(b, -1, h, h).unsqueeze(2)
            return CHW
    ...
    def forward(self, x):
        ...
        if self.s != 0:
            ...
            for i in range(0, self.rank):
                # broadcast to tensor1 shape
                tensor1 = tensor1 + local_lam[0].view(1)*self.TukerReconstruction(b, self.s, self.h , self.ps[0], self.conv1_1[i](C), None, self.conv1_3[i](H), self.conv1_4[i](W))
