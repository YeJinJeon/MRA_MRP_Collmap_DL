...
import sys
sys.path.append('/home/yejin/Projects/MRA_3D_MROD_Net')
...
class SceneUnderstandingModule(nn.Module):
    def __init__(self, n_channels, scale_factor, device, dilations=[1, 3, 6, 9]):
...
class EfficientnetBackboneDrnnF2LossOnly(nn.Module):
    ...
    def __init__(self, n_channels, growth_rate, reduction, k_ordinal_class, dev_0, dev_1, dev_2, dev_3, model_name, scale_factor=(1, 14, 14)):
        super(EfficientnetBackboneDrnnF2LossOnly, self).__init__()
        self.dev_0 = dev_0
        self.dev_1 = dev_1
        self.dev_2 = dev_2
        self.dev_3 = dev_3
        ...
        n_channels = 1280
        self.transpose_conv1 = nn.ConvTranspose3d(n_channels, int(n_channels * reduction), kernel_size=(3, 2, 2), stride=(1, 2, 2), padding=(1, 0, 0)).to(dev_1)
        self.scene_understanding_module = SceneUnderstandingModule(1280, scale_factor, device=dev_1).to(dev_1)
        # # resblock
        self.resblock1 = ResidualBlock(752).to(dev_1)
        n_channels = 1504
        # n_channels = int(n_channels * 6) # n_channels = 512
        self.transpose_conv2 = nn.ConvTranspose3d(n_channels, 120, kernel_size=(3, 2, 2), stride=(1, 2, 2), padding=(1, 0, 0)).to(dev_1)
        self.resblock2 = ResidualBlock(160).to(dev_1)
        n_channels = 320
        self.transpose_conv3 = nn.ConvTranspose3d(n_channels, 40, kernel_size=(3, 2, 2), stride=(1, 2, 2), padding=(1, 0, 0)).to(dev_1)
        ...
        self.ordinal_regression_conv2 = nn.Conv3d(48, k_ordinal_class * 10, kernel_size=(1, 1, 1)).to(dev_3)
        self.orl1 = OrdinalRegressionLayer().to(dev_2)
        self.orl2 = OrdinalRegressionLayer().to(dev_3)
    ...
    def forward(self, x):
        ...
        reduction_1, reduction_2, reduction_3, reduction_4, out = reduction_1.to(self.dev_1), reduction_2.to(self.dev_1), \
        ...
        out = out.to(self.dev_0)
        reg_output = self.last_conv(out)
        #
        reg_output = torch.tanh(reg_output)
        return reg_output.to(self.dev_0)


if __name__ == '__main__':
    a = torch.rand(1, 60, 25, 224, 224)
    net = EfficientnetBackboneDrnnF2LossOnly(60, 12, 0.5, 5, dev_0=torch.device("cpu"), dev_1=torch.device("cpu"), dev_2=torch.device("cpu"), dev_3=torch.device("cpu"), model_name="efficientnet-b0")
    # from utils.utils import count_parameters
