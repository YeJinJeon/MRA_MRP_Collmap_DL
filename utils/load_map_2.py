...
def load_phase_map(base_input, prefix, save_npy=False):
    ...
    if prefix == "dce":
        dir_phase_map = glob.glob(base_input + '/DCE_GFS3*')
        # if there are more than one collateral map directory - choose max number of slices(nos)
        if len(dir_phase_map) > 1:
            nos_list = [int(dir.split('/')[-1].split('_')[-1][-2:]) for dir in dir_phase_map]
            idx = np.argmax(nos_list)
            dir_phase_map = dir_phase_map[idx]
            nos = nos_list[idx]
        else:
            dir_phase_map = glob.glob(base_input + '/DCE_GFS3*')[0]
            nos = int(dir_phase_map.split('/')[-1].split('_')[-1][-2:])
    else:  # dsc
    ...
    def art_base(i, prefix):
        if i < 10:
            i = f"0{i}"
        if prefix == "dce":
            return glob.glob(f"{dir_phase_map}/DCE_Collateral_Arterial_0{i}.dcm")[0]
        else:
    ...
    def cap_base(i, prefix):
        if i < 10:
            i = f"0{i}"
        if prefix == "dce":
            return glob.glob(f"{dir_phase_map}/DCE_Collateral_Capillary_0{i}.dcm")[0]
        else:
    ...
    def even_base(i, prefix):
        if i < 10:
            i = f"0{i}"
        if prefix == "dce":
            return glob.glob(f"{dir_phase_map}/DCE_Collateral_Early_Venous_0{i}.dcm")[0]
        else:
    ...
    def lven_base(i, prefix):
        if i < 10:
            i = f"0{i}"
        if prefix == "dce":
            return glob.glob(f"{dir_phase_map}/DCE_Collateral_Late_Venous_0{i}.dcm")[0]
        else:
    ...
    def del_base(i, prefix):
        if i < 10:
            i = f"0{i}"
        if prefix == "dce":
            return glob.glob(f"{dir_phase_map}/DCE_Collateral_Delay_0{i}.dcm")[0]
        else:
    ...
    if prefix == "dce":
        ...
        rot_angle = 0
        sdi = 15
        dis = int(slice_info[-2].split("S")[:-2])
        tck = int(slice_info[-3].split("K")[:-2])
        nos = int(art.shape[2])
        r_slices = nos
