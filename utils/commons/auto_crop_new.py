...
def save_fig(mip: np.ndarray, idx: tuple, dir_npy: str, image_type=Constant.HEAD):
    """
    ...
    plt.savefig(f'{dir_npy}/../auto_crop_{image_type}.png', dpi=200)
    plt.close('auto_crop')
...
def detect_head_and_neck(mip: np.ndarray, thr: float, dir_npy: str):
    """
    Detect the head section from a maximum intensity projection image
    :param mip: maximum intensity projection array
    :param thr: threshold for removing the image background
    ...
    # Generating foreground mask
    thresh = threshold_otsu(mip)
    mask = mip > thresh

    # Detect head section
    idx_row, idx_col = np.where(mask == 1)
    crop_length = max(idx_row) - min(idx_row)
    # crop_length = min(max(crop_length, crop_range[0]), crop_range[1])
    # cut one seventh of width length from 2 vertical borders to make image smaller
    idx_row = np.arange(min(idx_row), min(idx_row) + crop_length)
    crop_width = max(idx_col) - min(idx_col)
    idx_col = np.arange(min(idx_col) + int(crop_width / 7), min(idx_col) + crop_width - int(crop_width / 7))
    # Save figure showing detected head section
    if dir_npy is not None:
        save_fig(mip, (idx_row, idx_col), dir_npy, image_type=Constant.HEAD_AND_NECK)

    return idx_row, idx_col

@timer
def auto_crop_head(img, dir_npy: str = None, pixel_height: float = 1., crop_range: tuple = (170, 180)):
    """
    ...
    idx_row, idx_col = detect_head(mip, -1, None, crop_range)
    idx_row[0], idx_row[-1] = max(0, idx_row[0] - 5), idx_row[-1] - 5
    idx_row[-1] += 1; idx_col[-1] += 1
    save_fig(mip, (idx_row, idx_col), dir_npy)
    # Crop head section from the original image
    img = img[:, :, idx_row[0]:idx_row[-1], idx_col[0]:idx_col[-1]]
    try:
    ...
    return img, idx_row, idx_col

@timer
def auto_crop_head_and_neck(img, idx_row, idx_col, dir_npy: str = None, pixel_height: float = 1.):
    """
    Detect the head section and return the 4D image containing only the head section
    :param img: 4D (or 3D) Numpy array
    :param dir_npy: a string indicating the saving folder for generated figures
    ...
    # Extract the maximum intensity projection image
    time_idx = np.argmax(img[:, :, :int(img.shape[2] / 2)].mean(axis=(1, 2, 3)))  # Only consider the top section
    idx_row[-1] = img.shape[2]
    mip = img[time_idx].max(axis=0)
    # Crop head and neck section from the original image
    save_fig(mip, (idx_row, idx_col), dir_npy, image_type=Constant.HEAD_AND_NECK)
    img = img[:, :, idx_row[0]: idx_row[-1], idx_col[0]:idx_col[-1]]
    ...
    #
    # # Detect head section from the maximum intensity projection image
    # idx_row, idx_col = detect_head_and_neck(mip, -1, dir_npy)
    # idx_row[0], idx_row[-1] = max(0, idx_row[0] - 5), idx_row[-1] - 5
    #
    # # Crop head section from the original image
    ...
    # try:
    #     for view in ['axial', 'coronal']:
    #         estimate_rotation_angle(img[time_idx], view, dir_npy, Constant.HEAD_AND_NECK)
    # except:
    #     pass
    #
    # return img
