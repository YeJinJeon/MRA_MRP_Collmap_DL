...
def save_color_fig(pred, save_path):
    ...
    for ind, phase in enumerate(['Art', 'Cap', 'EVen', 'LVen', 'Del']):
        ...
        fig.suptitle(phase)
        ax.imshow(montage(pred[ind][:25, :, :], grid_shape=(5, 5), multichannel= True))
        plt.figure('result')
    ...
    def predict(self):
        ...
        with open(self.error_file, 'w') as f:
            for index, datapath in enumerate(self.data_dirs):
                ...
                print(folder)
                if os.path.exists(os.path.join("/media/yejin/새 볼륨/compu/mra_v2/mrod_sdd/result/model_272_2022-09-22-15-45-29_seed42",folder)):
                    print("already")
                    continue
                try:
                    imgs, _, hdrs = load_dce(datapath, to_crop=True, auto_crop=True)
                except Exception as err:
                    f.write(f"{folder},Source:{err}\n")
                    continue
                try:
                    phases, properties = load_phase_map(datapath, prefix="dce")
                except Exception as err:
                    f.write(f"{folder},GT:{err}\n")
                    continue
                try:
                    image, label, mask = preprocess_dce(imgs, phases, properties)
                except Exception as err:
                    f.write(f"{folder},Preprocess:{err}\n")
                    continue
                ...
                # color_pred = mr_collateral_dce_gen_dicoms(directory, hdrs, _pred_array, _mask, suffix='DL', rescaling_first=True, insert_bar=True, from_deep_learning=True) # slice norm
...
if __name__ == '__main__':
    ...
    # predict
    root_dir = "/media/yejin/새 볼륨/compu/mra_v2/mrod_sdd"

    ### DATA
    csv_file = "./dataset/dasan2_dce_data.csv"
    ...
    model_checkpoint = "model_272.pth"
